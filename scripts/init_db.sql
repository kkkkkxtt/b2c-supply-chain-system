-- Database Setup Script (pgAdmin-compatible)
...
BEGIN
    RETURN QUERY
    INSERT INTO items (
        id, seller_id, item_name, description,
        price, stock, image_url, created_at
    )
    VALUES (
        p_id, p_seller_id, p_item_name,
        p_description, p_price, p_stock,
        p_image_url, NOW()
    )
    RETURNING *;
END;
